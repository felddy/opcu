...
class LPD8806(LEDStrip):
    ...
    def __apply_gamma(self, size):
        '''applies a gamma to values in ba_in,
        and sets values in ba_out.  Re-arranges colors to be RGB order.'''
        for i in range(size):
            self.__gamma_applied_buffer[i*3+0] = self.__gamma[self.buffer[i*3+1]]
            self.__gamma_applied_buffer[i*3+1] = self.__gamma[self.buffer[i*3+0]]
            self.__gamma_applied_buffer[i*3+2] = self.__gamma[self.buffer[i*3+2]]

    def __adafruit_gamma(self):
        '''returns the gamma table used in adafruit examples'''
        gamma = bytearray(256)
        for i in range(256):
...
def main():
    ...
    parser.add_argument('protocol', type=str, choices=['apa102','lpd8806'],
                        help='led strip protocol')
    # parser.add_argument('spidev', type=str, help='Serial Peripheral Interface bus device')
    parser.add_argument('-v', '--verbose', help='increase output verbosity',
                    action='store_true')
    parser.add_argument('-6', '--ipv6', help='listen on ipv6 ports',
    action='store_true')
    parser.add_argument('-p', '--port', type=int, default=DEFAULT_UDP_PORT,
                    help='port on which to listen')
    parser.add_argument('-b', '--bus', type=int, default=DEFAULT_SPIDEV_BUS,
                    help='spi bus number')
    parser.add_argument('-d', '--device', type=int, default=DEFAULT_SPIDEV_DEVICE,
                    help='spi device number')
    parser.add_argument('-m', '--max-led-count', type=int, help='Maximum number of LEDs on the strip',
    ...
    while running:
        ...
        if mode == StreamMode.HEADER:
            ...
            if args.verbose:
                print('got header: %s %s %s %s (%s)' % (h_channel, h_command,
                    h_length_hi, h_length_lo, pixel_data_length))
