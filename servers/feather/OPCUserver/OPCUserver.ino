...
//#define DEBUG // wait for serial connect when starting
...
#define IP_TYPE IP_DYNAMIC // IP_STATIC | IP_DYNAMIC | IP_BONJOUR
...
#endif
#define INPORT          7890 // Incoming UDP port to listen on
#define SPI_SPEED       10000000 // orig: 12000000
// Long DotStar stips may require reducing the SPI clock; if you see
// glitching, try setting to 8 MHz above.
#define DISABLE_DITHERING // disable dithering calculations

WiFiServer              server(INPORT);
WiFiUDP Udp;           // Set up a client session
...
#define MAX_LEDS        720 // Upper limit; OK to receive data for fewer
...
void magic(
 ...
 uint16_t numLEDs) { // Number of LEDs in buffer
  ...
  myDMA.changeDescriptor(desc, fillBuf);
  myDMA.startJob();
  spiReady = false;
}
...
void setup() {
  ...
  Serial.println("DEBUG MODE ENABLED: Requires serial monitor connection to start.");
  #endif
  ...
  desc = myDMA.addDescriptor(
    spiBuffer[spiBufferBeingFilled],  // Source address
...
Serial.println("LED Dithering is disabled.");
#else
Serial.println("LED Dithering is enabled.");
#endif
  ...
  Serial.print("UDP server listening at ");
...
void loop() {
  ...
  if(true) {
    ...
    uint8_t  w;
    Serial.println("Starting receive loop.");
    while(true) { //was tcp client.connected
      ...
      if(bytesPending = Udp.parsePacket()) {  // Any incoming data?
        ...
        do {
          if(mode == MODE_DATA) { // Receiving pixel data, most likely case
            ...
            if((a = Udp.read(&rgbBuf[bufBeingRead][bytesRead],
             bytesPending)) > 0) {
              bytesPending -= a;
              bytesToRead  -= a;
            ...
            if((a = Udp.read(&rgbBuf[3][0], bytesPending)) > 0) { // Poof!
              bytesPending -= a;
    ...
    //XXX Never get here with UDP version
    // client.stop();
